    ...
    static void init(Database database) throws DatabaseException {
        synchronized (lock) {
            BanHandler.db = database;
                        try {
                synchronized (db.getConnection()) {db.execute("CREATE TABLE IF NOT EXISTS " + getTable() + "(" +
